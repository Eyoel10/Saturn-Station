...
public class StablizeBalls : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        weights = new int[] {1,2,1,1,2,2};

        StartCoroutine(MakeFirstBubbles(0.2f, 0, 0, 10));
    }
    ...
    void Update()
    {
        ...
        if (firstBubblesPlaced && !isLevelingUp)
        {
            ...
            // Update the place of empty spots in the scene
            ...
            if (!goodRandFound)
            {
                ...
                randomBall = GetWeightedRandomIndex(weights);
    ...
    {
        ClearAvailableSpots();
        // Update the position of all the empty spots in the scene
        for (int i = 0; i < balls.GetLength(0); i++)
        {
            for (int j = 0; j < balls.GetLength(1); j++)
            {
    ...
    int getRandom()
    {
        return availableSpots[UnityEngine.Random.Range(0, availableSpots.Count)];
    }
    ...
    public IEnumerator MakeFirstBubbles(float duration, int level, int placementPoint, int count)
    {
        ...
        if (count > 0)
        {
            ...
            MakeNewBall(randomBall, placementPoint, level, placementPoint);
            
            
            yield return StartCoroutine(MakeFirstBubbles(duration, level+1, placementPoint, count - 1));
        }
        else
        {
            firstBubblesPlaced = true;
        }
    }
    ...
    public int GetWeightedRandomIndex(int[] weight)
    {
        int totalWeight = 0;
